...
# ¿Como demuestro que usa y,z,x?
v  = 2024
L = 160 # Numero de bits
n = 20 #Longitud de los vectores, preguntar a Martin de donde sale esta n 
...
def Alice1(v):
    ...
    # 5. mu = delta + rho*x 
    mu = (alpha + rho*x) % p 
    return (gamma,V, aL, aR, alpha, A, sL, sR, rho, S, y, z,t0, t1, t2, tao1, tao2, T1,T2,x,l,r,t,taox, mu)
...
def Bob3(gamma,V, aL, aR, alpha, A, sL, sR, rho, S, y, z,t0, t1, t2, tao1, tao2, T1,T2,x,l,r,t,taox, mu):
    ...
    # 4. Cuarta Igualdad, comprobar que Alice ha usado las Hash que se supone que tiene que usar
    y1m = str(V)+str(A)+str(S)+ str(1)
    y1m1 = y1m.encode('utf-8')
    y1 = (int(hashlib.sha1(y1m1).hexdigest(), base=16))%p
    if y1 != y:
       print("error4")
       
    z1m = str(V)+str(A)+str(S)+ str(2)
    z1m1 = z1m.encode('utf-8')
    z1 = (int(hashlib.sha1(z1m1).hexdigest(), base=16))%p
    if z1 != z:
       print("error5")
       
    x1m = str(T1)+str(T2)
    x1m1 = x1m.encode('utf-8')
    x1 = (int(hashlib.sha1(x1m1).hexdigest(), base=16))%p 
    if x1 != x:
       print("error6")
     
    print("ok")
 
(gamma,V, aL, aR, alpha, A, sL, sR, rho, S, y, z,t0, t1, t2, tao1, tao2, T1,T2,x,l,r,t,taox, mu) = Alice1(v)
